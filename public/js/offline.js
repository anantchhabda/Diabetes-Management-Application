...
(function () {
  ...
  async function enqueue(resource, payload) {
    ...
    await qPut(rec);
    showBanner(
      `Saved offline — ${resource} entry for ${
        payload?.date || "today"
      ...
      for (const m of batch) {
        ...
        try {
          ...
          if (err && (err.status === 401 || err.status === 403)) {
            ...
            showBanner(
              "Session expired. Re-login required to sync offline changes."
            );
            break;
          }
  ...
  function onOnline() {
    showBanner("You're back online. Syncing saved data...");
    flush();
  }
  ...
  function register(resource, pushFn) {
    handlers.set(resource, pushFn);
  }
  ...
  function showBanner(msg) {
    ...
    if (el) {
      ...
      console.log("[offline-banner]", msg);
    }
  }
