...
//i18n helpers preserved from main
  ...
  // map nepalese digits
  const NE_DIG = { 0: "०", 1: "१", 2: "२", 3: "३", 4: "४", 5: "५", 6: "६", 7: "७", 8: "८", 9: "९" };
  ...
  function getAuthToken() {
    ...
    if (!token) throw new Error(t("session_expired", "Session expired, please register again"));
    return token;
  }
  ...
  async function createReminder(reminder) {
    ...
    if (!res.ok) throw new Error(data?.error || text || "Failed to create reminder");
    return data;
  }
  ...
  function weekdayLong(date) {
    if (!date) return "";
    try {
      return new Date(date + "T00:00:00").toLocaleDateString(currentLang(), { weekday: "long" });
    } catch {
      return "";
    }
  }
        ...
        <div class="time-picker-popup hidden absolute top-full left-0 mt-1 bg-[var(--color-secondary)] border border-[var(--color-textWhite)] rounded-lg p-4 z-50 shadow-lg">
          <div class="flex gap-4 items-center">
            <div class="flex flex-col items-center">
              <label class="text-[var(--color-textWhite)] text-sm mb-2">${t("tp_hour","Hour")}</label>
              <div class="hour-selector h-32 overflow-y-scroll border border-[var(--color-textWhite)] rounded w-16">
                ${Array.from({ length: 24 }, (_, i) => {
                  const hour = i.toString().padStart(2, "0");
                  const label = currentLang() === "ne" ? toNepaliDigits(hour) : hour;
            ...
            <div class="flex flex-col items-center">
              <label class="text-[var(--color-textWhite)] text-sm mb-2">${t("tp_minute","Minute")}</label>
              <div class="minute-selector h-32 overflow-y-scroll border border-[var(--color-textWhite)] rounded w-16">
                ${Array.from({ length: 60 }, (_, i) => {
                  const minute = i.toString().padStart(2, "0");
                  const label = currentLang() === "ne" ? toNepaliDigits(minute) : minute;
          ...
          <div class="flex gap-2 mt-4">
            <button type="button" class="time-cancel flex-1 bg-gray-300 text-[var(--color-textBlack)] rounded py-1 font-semibold">${t("cancel","Cancel")}</button>
            <button type="button" class="time-confirm flex-1 bg-[var(--color-tertiary)] text-[var(--color-textWhite)] rounded py-1 font-semibold">${t("confirm","Confirm")}</button>
          </div>
        </div>
      </div>
    `;
  ...
  function setupTimePickerEvents(popup) {
    popup.addEventListener("click", function (e) {
      ...
      if (e.target.classList.contains("hour-option")) {
        ...
        hourSelector.querySelectorAll(".hour-option").forEach((o) => o.classList.remove("bg-[var(--color-tertiary)]"));
      ...
      if (e.target.classList.contains("minute-option")) {
        ...
        minuteSelector.querySelectorAll(".minute-option").forEach((o) => o.classList.remove("bg-[var(--color-tertiary)]"));
        e.target.classList.add("bg-[var(--color-tertiary)]");
      }
    ...
    document.addEventListener("click", function (e) {
      if (!e.target.closest(".custom-time-picker")) {
        popup.querySelectorAll(".time-picker-popup").forEach((picker) => picker.classList.add("hidden"));
      }
    });
  }
  ...
  async function renderReminders() {
    ...
    if (!container) {
      ...
      container.className = "w-full max-w-md space-y-2 flex flex-col items-center";
    ...
    container.innerHTML = reminders
      .map((r) => {
        ...
        <div class="flex border rounded overflow-hidden w-full h-20">
          <div class="bg-[var(--color-secondary)] text-[var(--color-textWhite)] flex items-center justify-center px-4 w-32 text-sm font-bold text-center">${r.name || ""}</div>
          <div class="flex-1 flex flex-col justify-center px-3 py-2" style="background:#1b7fa6;">
            ...
            <div class="flex gap-1">
              <button class="bg-[var(--color-tertiary)] text-[var(--color-textWhite)] px-2 py-1 rounded font-semibold text-xs editReminderBtn" data-id="${r._id}">${t("edit","Edit")}</button>
              <button class="bg-[var(--color-delete)] text-[var(--color-textWhite)] px-2 py-1 rounded font-semibold text-xs removeReminderBtn" data-id="${r._id}">${t("remove","Remove")}</button>
            </div>
          </div>
        </div>`;
      })
  ...
  function relocalizeOpenPopup(popup) {
    ...
    if (h2) {
      const editing = !!popup.querySelector("#reminderForm")?.dataset.reminderId;
      h2.textContent = editing ? t("edit_reminder","Edit Reminder") : t("new_reminder","New Reminder");
    }
    ...
    const hourLabel = popup.querySelector(".hour-selector")?.closest("div")?.querySelector("label");
    if (hourLabel) hourLabel.textContent = t("tp_hour","Hour");
    const minuteLabel = popup.querySelector(".minute-selector")?.closest("div")?.querySelector("label");
    ...
    if (timeInput) {
      ...
      if (timeInput.dataset.time) timeInput.value = formatTimeDisplay(timeInput.dataset.time);
      popup.querySelectorAll(".hour-option").forEach((el) => el.textContent = currentLang()==="ne" ? toNepaliDigits(el.dataset.hour) : el.dataset.hour);
      popup.querySelectorAll(".minute-option").forEach((el) => el.textContent = currentLang()==="ne" ? toNepaliDigits(el.dataset.minute) : el.dataset.minute);
    }
    popup.querySelectorAll(".time-cancel").forEach((b)=> b.textContent = t("cancel","Cancel"));
    popup.querySelectorAll(".time-confirm").forEach((b)=> b.textContent = t("confirm","Confirm"));
  ...
  function showReminderPopup(reminder = {}, reminderID = null) {
    ...
    popup.innerHTML = `
      <div class="bg-[var(--color-secondary)] rounded-lg p-6 w-80 relative shadow-lg text-[var(--color-textWhite)]">
        <h2 class="text-xl font-bold mb-4 text-[var(--color-textWhite)]">
          ${reminderID ? t("edit_reminder","Edit Reminder") : t("new_reminder","New Reminder")}
        </h2>
        <form id="reminderForm" class="flex flex-col gap-3" ${reminderID ? `data-reminder-id="${reminderID}"` : ""}>
          <input type="text" name="name" placeholder="${t("reminder_name_ph","Name...")}" required
          ...
          <select name="interval" required class="w-full rounded px-2 py-1 bg-transparent border border-[var(--color-textWhite)] text-[var(--color-textWhite)]">
            <option value="">${t("interval_placeholder","Interval/Repeat")}</option>
            <option value="Daily" ${reminder.interval === "Daily" ? "selected" : ""}>${t("interval_daily","Daily")}</option>
            <option value="Weekly" ${reminder.interval === "Weekly" ? "selected" : ""}>${t("interval_weekly","Weekly")}</option>
            <option value="Monthly" ${reminder.interval === "Monthly" ? "selected" : ""}>${t("interval_monthly","Monthly")}</option>
          </select>
          <div class="flex gap-2 mt-4">
            <button type="button" id="cancelReminderBtn" class="flex-1 bg-gray-300 text-[var(--color-textBlack)] rounded py-1 font-semibold">${t("cancel","Cancel")}</button>
            <button type="submit" class="flex-1 bg-[var(--color-tertiary)] text-[var(--color-textWhite)] rounded py-1 font-semibold">${t("save","Save")}</button>
          </div>
        </form>
      </div>
    `;
    ...
    document.getElementById("reminderForm").onsubmit = async function (e) {
      ...
      const payload = { name, date, time, interval };
  ...
  function showDeletePopup(reminderID) {
    ...
    popup.innerHTML = `
      <div class="bg-[var(--color-secondary)] rounded-lg p-6 w-80 relative shadow-lg text-[var(--color-textWhite)]">
        <h2 class="text-xl font-bold mb-4 text-[var(--color-textWhite)]">${t("delete_reminder","Delete Reminder")}</h2>
        <p class="mb-6 text-[var(--color-textWhite)]">${t("delete_confirm_text","Are you sure you want to delete this reminder?")}</p>
        <div class="flex gap-2">
          <button type="button" id="cancelDeleteBtn" class="flex-1 bg-gray-300 text-[var(--color-textBlack)] rounded py-1 font-semibold">${t("cancel","Cancel")}</button>
          <button type="button" id="confirmDeleteBtn" class="flex-1 bg-[var(--color-delete)] text-[var(--color-textWhite)] rounded py-1 font-semibold">${t("delete","Delete")}</button>
        </div>
      </div>
    `;
    document.body.appendChild(popup);
    ...
    observeLangChanges(() => {
      ...
      if (p) p.textContent = t("delete_confirm_text","Are you sure you want to delete this reminder?");
